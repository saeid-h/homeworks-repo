...
%             for camera2 = [prev next]
                ...
                if p_image2(1) < 1 || p_image2(1) >= height || ...
                    ...
                    if SAD < treshold
                        votes{agreement}.camera1 = camera1;
                        votes{agreement}.camera2 = camera2;
                        votes{agreement}.coordinate = voxel(node,1:3);
                        votes{agreement}.RGB1 = [R1 G1 B1];
                        votes{agreement}.RGB2 = [R2 G2 B2];
                        votes{agreement}.SAD = SAD;
    ...
    if agreement < 2
        ...
        cameras = zeros(1,8);
        for j = 1:numel(votes)
            cameras(votes{j}.camera1) = cameras(votes{j}.camera1) + 1;
            cameras(votes{j}.camera2) = cameras(votes{j}.camera2) + 1;
        end
        camera = find(cameras==max(cameras));
        if numel(camera) > 1
            camera = camera(1);
        end
        j = 1;
        while votes{j}.camera1 ~= camera && ...
                votes{j}.camera2 ~= camera
            j = j + 1;
        end
%         if votes{j}.camera1 == camera
%             RGB = votes{j}.RGB1;
%         else
%             RGB = votes{j}.RGB2;
%         end
        voxel(node, 4:6) = uint8(RGB);
    end %if
