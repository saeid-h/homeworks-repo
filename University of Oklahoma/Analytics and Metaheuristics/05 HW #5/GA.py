...
def initializePopulation(): #n is size of population; d is dimensions of chromosome
    ...
    #for example:  popVals[0] is repre sents the best individual in the population
...
def mutate(x):
    ...
    xx=x[:]
    altIndex= myPRNG.randint(0, dimensions-1)
    #x[altIndex] = x[altIndex]*-1
    a = myPRNG.randrange(lowerBound, upperBound)
...
def breeding(matingPool):
    ...
    for i in range(0,populationSize-1,2):
        ...
        if (myPRNG.random() < crossOverRate):
            ...
            #print("Mutation")
            child1=mutate(matingPool[i])
            child2=mutate(matingPool[i+1])
...
def plot(pop):
    ...
    for l in range(populationSize):
        xxx.append(Population[l][0][0])
        yyy.append(Population[l][0][1])
        zzz.append(Population[l][1])
